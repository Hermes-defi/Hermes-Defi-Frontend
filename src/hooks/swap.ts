...
export function useSwapUSDCForPlutus() {
  const { account } = useActiveWeb3React();
  const swapContract = useSwapUSDCContract();
  const queryClient = useQueryClient();
  const toast = useToast();
  ...
  const mutation = useMutation(
    ...
    {
      onSuccess: async () => {
        await queryClient.invalidateQueries("plutusInWallet");

        ReactGA.event({
          category: "Swap USDC for PLUTUS",
          action: `Swapped USDC for PLUTUS`,
        });
      },

      onError: ({ data }) => {
        console.error(`[useSwapUSDCForPlutus][error] general error `, {
          data,
        });

        toast({
          title: "Error purchasing PLUTUS",
          description: data?.message,
          status: "error",
          position: "top-right",
          isClosable: true,
