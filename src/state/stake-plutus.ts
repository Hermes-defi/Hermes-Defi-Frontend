...
import { PlutusStakeInfo, plutusStakingPools } from "config/stake-plutus";
...
function useFetchStakingPoolRequest() {
  ...
  return async (stakePoolInfo: PlutusStakeInfo) => {
    try {
...
export function useFetchStakePools() {
  ...
  const farmQueries = useQueries(
    plutusStakingPools.map((stakePool) => {
      return {
        enabled: !!currentBlock,
        queryKey: ["pre-plutus-stake", stakePool.address, account],
...
export function useApproveStakePool() {
  ...
  const approveMutation = useMutation(
    async (address: string) => {
      ...
      const pool = queryClient.getQueryData<PlutusStakeInfo>(["pre-plutus-stake", address, account]);
    ...
    {
      onSuccess: (address) => {
        const stakePool = queryClient.getQueryData<PlutusStakeInfo>(["pre-plutus-stake", address, account]);

        queryClient.setQueryData(["pre-plutus-stake", address, account], {
          ...stakePool,
...
export function useDepositIntoStakePool() {
  ...
  const depositMutation = useMutation(
    async ({ id, amount }: { id: string; amount: string }) => {
      ...
      const pool = queryClient.getQueryData<PlutusStakeInfo>(["pre-plutus-stake", id, account]);
    ...
    },
    {
      onSuccess: (_, { id, amount }) => {
        const pool = queryClient.getQueryData<PlutusStakeInfo>(["pre-plutus-stake", id, account]);
        queryClient.invalidateQueries(["pre-plutus-stake", id, account]);
